...
class ForegroundSyncService {
  ...
  async start(): Promise<void> {
    ...
    // IMPORTANTE: Recordar al usuario sobre optimización de batería (solo la primera vez)
    // Esto es CRÍTICO para que funcione cuando cambie de app
    this.showBatteryOptimizationReminderOnce();
  ...
  /**
   * Muestra un recordatorio sobre optimización de batería solo la primera vez.
   * Esto es CRÍTICO para que la sincronización funcione cuando el usuario cambia de app.
   */
  private async showBatteryOptimizationReminderOnce(): Promise<void> {
    const STORAGE_KEY = '@battery_optimization_reminder_shown';
    ...
    try {
      ...
      if (alreadyShown === 'true') {
        console.log(
          '[ForegroundSync] ℹ️ Recordatorio de batería ya mostrado anteriormente',
        );
        return;
      }

      console.log(
        '[ForegroundSync] 🔋 Mostrando recordatorio de optimización de batería...',
      );

      // Marcar como mostrado ANTES de mostrar (evita duplicados)
      await AsyncStorage.setItem(STORAGE_KEY, 'true');

      // Mostrar el recordatorio después de un pequeño delay para no interrumpir el inicio
      setTimeout(() => {
        NotificationPermissionService.requestBatteryOptimizationExclusion();
      }, 3000);
    } catch (error) {
      console.error(
        '[ForegroundSync] Error verificando recordatorio de batería:',
        error,
      );
    }
  }
